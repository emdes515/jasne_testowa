    ...
    <div className="flex flex-col p-4 sm:p-6 min-h-full pb-[140px] max-w-3xl mx-auto w-full relative">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[#8B8D98] text-xs sm:text-sm">Oficjalne arkusze CKE • Nowa Formuła 2023</p>
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {view === 'menu' && (
            ...
            >
              ...
              >
                <div className="absolute inset-0 bg-surface-bg rounded-[24px] translate-y-2 transition-transform group-active:translate-y-0"></div>
                <div className="relative bg-surface-card border border-surface-border rounded-[24px] p-6 shadow-lg group-hover:border-[#3B82F6]/30 transition-colors h-full">
                  ...
                  <h3 className="font-display font-bold text-text-primary text-lg mb-2 tracking-wide">Pojedyncze zadania</h3>
              ...
              >
                <div className="absolute inset-0 bg-surface-bg rounded-[24px] translate-y-2 transition-transform group-active:translate-y-0"></div>
                <div className="relative bg-surface-card border border-surface-border rounded-[24px] p-6 shadow-lg group-hover:border-accent-ai/30 transition-colors h-full">
                  <div className="absolute top-0 right-0 w-32 h-32 bg-accent-ai/10 rounded-full blur-[40px] -translate-y-1/2 translate-x-1/4 pointer-events-none"></div>
                  <div className="flex items-start justify-between relative z-10">
                    <div className="w-14 h-14 bg-gradient-to-br from-accent-ai to-purple-800 p-[2px] rounded-2xl mb-4 shadow-[0_0_15px_rgba(168,85,247,0.3)]">
                      <div className="w-full h-full bg-surface-card rounded-2xl flex items-center justify-center">
                        <BookOpen className="text-accent-ai" size={24} />
                      </div>
                    </div>
                    <span className="text-[10px] uppercase font-black tracking-widest text-accent-ai bg-accent-ai/10 px-3 py-1.5 rounded-lg border border-accent-ai/20 flex items-center gap-1.5 shadow-sm">
                      <Clock size={12} /> 20 MIN
                    </span>
                  </div>
                  <h3 className="font-display font-bold text-text-primary text-lg mb-2 tracking-wide relative z-10">Mini Matura (Próbny arkusz)</h3>
                  <p className="text-text-secondary text-sm leading-relaxed font-medium relative z-10">
              ...
              >
                <div className={`absolute inset-0 bg-surface-bg rounded-[24px] ${mistakesBank.length > 0 ? 'translate-y-2 group-active:translate-y-0' : 'translate-y-0'} transition-transform`}></div>
                <div className={`relative bg-surface-card border border-surface-border rounded-[24px] p-6 shadow-lg h-full ${mistakesBank.length > 0 ? 'group-hover:border-primary/30' : 'opacity-60'} transition-colors`}>
                  <div className="flex items-start justify-between">
                    <div className={`w-14 h-14 rounded-2xl p-[2px] mb-4 ${mistakesBank.length > 0 ? 'bg-gradient-to-br from-[#FFC72C] to-primary shadow-[0_0_15px_rgba(255,184,0,0.3)]' : 'bg-white/10'}`}>
                      <div className="w-full h-full bg-surface-card rounded-2xl flex items-center justify-center">
                        <RotateCcw className={mistakesBank.length > 0 ? 'text-primary' : 'text-text-muted'} size={24} />
                      </div>
                    </div>
                    <span className={`text-[10px] uppercase font-black tracking-widest px-3 py-1.5 rounded-lg border shadow-sm ${mistakesBank.length > 0 ? 'bg-primary/10 text-primary border-primary/20' : 'bg-white/5 text-text-muted border-white/10'}`}>
                      {mistakesBank.length} DO POWTÓRKI
                    </span>
                  </div>
                  <h3 className="font-display font-bold text-text-primary text-lg mb-2 tracking-wide">Baza Błędów</h3>
                  <p className="text-text-secondary text-sm leading-relaxed font-medium">
