...
export default function App() {
  ...
  const handleCompleteTask = async (
    ...
    if (activeTaskData?.lessonId || lessonKey) {
      const lid = activeTaskData?.lessonId || lessonKey;
      const cleanLid = String(lid).replace('lesson-', '').replace('-', '.');
      const updatedMistakes = {
        ...lessonMistakes,
        [String(lid)]: mistakesCount,
        [cleanLid]: mistakesCount,
