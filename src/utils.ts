...
/**
 * Checks if an ID string represents an actual practice task,
 * as opposed to a lesson identifier, lesson marker tag, or theory section.
 */
export function isActualTaskId(id: string): boolean {
  if (!id || typeof id !== 'string') return false;
  const trimmed = id.trim();
  if (!trimmed) return false;
  
  // Exclude lesson tags and prefixes
  if (trimmed.startsWith('LESSON-') || trimmed.startsWith('lesson-')) return false;
  if (trimmed.startsWith('THEORY-') || trimmed.includes('THEORY') || trimmed.startsWith('theory-')) return false;
  if (trimmed.startsWith('exam-dzial-') || trimmed.startsWith('exam-')) return false;
  
  // Exclude bare lesson numbers like "1.1", "1-1", "2.14", "3-5"
...
/**
 * Returns only unique, valid task IDs representing completed practice tasks.
 */
export function filterActualTaskIds(taskIds: (string | undefined | null)[]): string[] {
  if (!Array.isArray(taskIds)) return [];
  const valid = taskIds.filter((id): id is string => Boolean(id && isActualTaskId(id)));
