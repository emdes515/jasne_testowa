...
export function normalizeTask(task: any, lesson: any, topic: any): any {
  ...
  return {
    ...
    title: lesson ? `Lekcja ${lesson.id.replace(/^(?:pol|eng|mat-roz|math-roz|eng-roz)?[-_]?lesson[-_]?/i, '').replace(/-/g, '.')}: ${lesson.title}` : 'Zadanie',
    topic: topic ? `${topic.short_title || topic.title || topic.name || 'Matematyka'} • ${lesson?.title || ''}` : 'Matematyka',
  ...
  lessonsDocs.forEach(lesson => {
    ...
    if (lesson.theory_pill) {
      ...
      allTasks.push({
        ...
        title: `Lekcja ${lesson.id.replace(/^(?:pol|eng|mat-roz|math-roz|eng-roz)?[-_]?lesson[-_]?/i, '').replace(/-/g, '.')}: ${lesson.title}`,
        topic: `${topicDoc.short_title || topicDoc.title || topicDoc.name || 'Matematyka'} • Lekcja ${lesson.id.replace(/^(?:pol|eng|mat-roz|math-roz|eng-roz)?[-_]?lesson[-_]?/i, '').replace(/-/g, '.')}`,
