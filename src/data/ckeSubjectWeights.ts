import { SubjectCkeConfig, TopicCkeWeight } from '../types';

export const CKE_SUBJECTS_CONFIG: Record<string, SubjectCkeConfig> = {
  ...
  'jezyk-polski': {
    ...
    topics: {
      ...
      'pol-dzial-7': { topicId: 'pol-dzial-7', name: 'Notatka syntetyzująca – Warsztat mistrzowski (3 pkt CKE)', minPoints: 3, maxPoints: 3, averagePoints: 3.0, importance: 'CRITICAL_PEWNIAK' },
      ...
      'pol-dzial-10': { topicId: 'pol-dzial-10', name: 'Renesans, Barok i Oświecenie – Od harmonii po teatr świata i satyrę', minPoints: 2, maxPoints: 5, averagePoints: 3.5, importance: 'HIGH' },
      ...
      'pol-dzial-14': { topicId: 'pol-dzial-14', name: 'Dwudziestolecie międzywojenne – Niepodległość, mit formy i rozliczenie z iluzjami', minPoints: 3, maxPoints: 12, averagePoints: 7.0, importance: 'CRITICAL_PEWNIAK' },
      'pol-dzial-15': { topicId: 'pol-dzial-15', name: 'Literatura wojny i okupacji – Zagłada i totalitaryzmy', minPoints: 4, maxPoints: 15, averagePoints: 8.0, importance: 'CRITICAL_PEWNIAK' },
      'pol-dzial-16': { topicId: 'pol-dzial-16', name: 'Literatura po 1945 r. – Wobec totalitaryzmu, władzy i kryzysu wartości', minPoints: 4, maxPoints: 15, averagePoints: 8.0, importance: 'CRITICAL_PEWNIAK' },
      'pol-dzial-17': { topicId: 'pol-dzial-17', name: 'Literatura współczesna – Pamięć, tożsamość, prowincja i spotkanie z Innym', minPoints: 2, maxPoints: 10, averagePoints: 5.0, importance: 'HIGH' }
    }
  }
};

/**
 * Zwraca konfigurację wag CKE dla danego przedmiotu z bezpiecznym fallbackiem
 */
export function getSubjectCkeConfig(subjectId?: string): SubjectCkeConfig {
  ...
  return CKE_SUBJECTS_CONFIG[normId] || CKE_SUBJECTS_CONFIG['matematyka-podstawowa'];
}
...
export const CKE_AVAILABLE_SUBJECTS: CkeSubjectOption[] = [
  {
  ...
  {
    id: 'biologia-rozszerzona',
    key: 'bio',
    shortName: 'Biologia',
    fullName: 'Biologia (Poziom Rozszerzony)',
  ...
  {
    id: 'chemia-rozszerzona',
    key: 'chem',
    shortName: 'Chemia',
    fullName: 'Chemia (Poziom Rozszerzony)',
    examTag: 'Rozszerzenie • 60 pkt',
    totalPoints: 60,
    passingPoints: 18,
    accentColor: '#8B5CF6',
...
export function getSubjectMetaByKey(key: string): CkeSubjectOption {
  const found = CKE_AVAILABLE_SUBJECTS.find(s => s.key === key || s.id === key);
  return found || CKE_AVAILABLE_SUBJECTS[0];
}
