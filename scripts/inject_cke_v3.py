...
files = {
    ...
    lessons[0]["tasks"].append(
        make_open_task(
            ...
            question="Rozwiąż równanie $$\\frac{3x - 6}{x - 2} = x + 1$$ Zapisz obliczenia.",
            points=2,
            scoring_key=[
...
''',
    r"scripts\curriculum_builder\topic_11_builder.py": r'''
    # --- Wstrzyknięte z arkuszy CKE ---
...
for path, injection in files.items():
    ...
    # Remove previous injections first
    # Find "# --- Wstrzyknięte z arkuszy CKE ---" and remove it and everything up to "return {"
    if "# --- Wstrzyknięte z arkuszy CKE ---" in code:
        code = re.sub(r'# --- Wstrzyknięte z arkuszy CKE ---.*?return\s*\{', 'return {', code, flags=re.DOTALL)
...
print("Injections fixed successfully.")
