...
// Adversarial finding: check whether any unbalanced $ delimiters remain
if (unbalancedDollarFields.length > 0) {
  console.warn(`\n  [ADVERSARIAL VULNERABILITY] Found ${unbalancedDollarFields.length} text fields with unbalanced / stray $ delimiters!`);
  console.warn('  Examples:');
  unbalancedDollarFields.slice(0, 5).forEach((e, idx) => {
    console.warn(`    ${idx + 1}. [${e.path}]: "${e.text.slice(0, 100)}..."`);
  });
}
...
console.log(`ADVERSARIAL FINDINGS:  ${unbalancedDollarFields.length} fields with unbalanced math delimiters`);
console.log('================================================================');
...
if (failedTests > 0) {
  ...
  console.log('\n[RESULT: PASS] - All 5 core milestone objectives passed empirical verification.');
  process.exit(0);
}
